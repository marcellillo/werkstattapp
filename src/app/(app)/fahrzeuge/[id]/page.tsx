...
export default async function FahrzeugDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [
    ...
    supabase.from('betrieb_einstellungen').select('wert').eq('betrieb_id', betriebId).eq('schluessel', 'fahrzeug_steuerart_standard').maybeSingle(),
  ])
  ...
  return (
    <>
      <SetTitle value={`${(auftrag as any).fahrzeug?.marke} ${(auftrag as any).fahrzeug?.modell}`} />
      <FahrzeugDetail
