...
import { Users, Mail, Plus, Copy, Check, Loader2, Calendar, Trash2 } from 'lucide-react'
...
export function MitarbeiterContent({ betriebId, users, invitations }: Props) {
  ...
  const handleDeleteInvitation = async (invitationId: string, email: string) => {
    if (!confirm(`Einladung an ${email} wirklich löschen?`)) return
    ...
    try {
      const response = await fetch('/api/invitations/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ invitationId, betriebId }),
      })
  ...
  const handleRemoveMember = async (membershipId: string, name: string) => {
    ...
    try {
      const response = await fetch('/api/team/remove', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ membershipId, betriebId }),
      })
